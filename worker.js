...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === "/health") {
      return cors(json({ok:true,service:"snowtech-saj-api",version:"0.10.8"}), env);
    }
...
async function getText(url){
  const r=await fetch(url,{headers:{
    "User-Agent":"SnowTech/0.10.8 (+public SAJ data lookup)",
    ...
    if([301,302,303,307,308].includes(r.status)){
      ...
      if([301,302,303].includes(r.status)){
        currentInit={method:"GET",headers:{
          "User-Agent":"SnowTech/0.10.8 (+public SAJ competition calendar lookup)",
          "Accept":"text/html,application/xhtml+xml"
        }};
      }
      continue;
