...
const SAJ_RANKING_CACHE_VERSION = "v01338";
...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === "/health") {
      return cors(json({ok:true,service:"snowtech-saj-api",version:"0.13.38"}), env, request);
    }
...
async function getText(url){
  const r=await fetch(url,{headers:{
    "User-Agent":"AlpineTeamManager/0.13.38 (+public SAJ data lookup)",
    "Accept":"text/html,application/xhtml+xml"
  }});
    ...
    if([301,302,303,307,308].includes(r.status)){
      ...
      if([301,302,303].includes(r.status)){
        currentInit={method:"GET",headers:{
          "User-Agent":"AlpineTeamManager/0.13.38 (+public SAJ competition calendar lookup)",
          "Accept":"text/html,application/xhtml+xml"
        }};
      }
      continue;
