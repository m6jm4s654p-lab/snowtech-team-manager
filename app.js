...
// ===== v0.13.64 National SAJ point ranking: persistent weekly cache =====
let nationalRankingState={category:'k2',sex:'男',discipline:'SL'};
const nationalRankingCache=new Map();
const NATIONAL_RANKING_STORAGE_KEY='alpine_team_manager_national_top30_v2';
const NATIONAL_RANKING_COMBOS=[['k2','男','SL'],['k2','男','GS'],['k2','男','SG'],['k2','女','SL'],['k2','女','GS'],['k2','女','SG'],['general','男','SL'],['general','男','GS'],['general','男','SG'],['general','女','SL'],['general','女','GS'],['general','女','SG']];
function nationalRankingKey(s=nationalRankingState){return `${s.category}|${s.sex}|${s.discipline}`;}
function loadNationalRankingCacheFromStorage(){try{const x=JSON.parse(localStorage.getItem(NATIONAL_RANKING_STORAGE_KEY)||'null');nationalRankingCache.clear();Object.entries(x?.data||{}).forEach(([k,v])=>{if(v)nationalRankingCache.set(k,v)});return x&&typeof x==='object'?x:null}catch(e){console.warn('全国TOP30キャッシュ読込失敗',e);return null}}
function persistNationalRankingCache(){const data={};nationalRankingCache.forEach((v,k)=>data[k]=v);try{localStorage.setItem(NATIONAL_RANKING_STORAGE_KEY,JSON.stringify({updatedAt:new Date().toISOString(),data}))}catch(e){console.warn('全国TOP30キャッシュ保存失敗',e)}}
function nationalRankingAutoSeason(now=new Date()){const m=now.getMonth()+1;return m>=11||m<=4;}
function nationalRankingLatestThursdayStart(now=new Date()){const d=new Date(now.getFullYear(),now.getMonth(),now.getDate());d.setDate(d.getDate()-((d.getDay()-4+7)%7));d.setHours(0,0,0,0);return d;}
function nationalRankingNeedsWeeklyRefresh(stored,now=new Date()){if(!nationalRankingAutoSeason(now))return false;if(!stored?.updatedAt)return true;if(NATIONAL_RANKING_COMBOS.some(([category,sex,discipline])=>!nationalRankingCache.has(nationalRankingKey({category,sex,discipline}))))return true;const d=new Date(stored.updatedAt);return Number.isNaN(d.getTime())||d<nationalRankingLatestThursdayStart(now);}
function syncNationalRankingButtons(){document.querySelectorAll('[data-ranking-category]').forEach(b=>b.classList.toggle('active',b.dataset.rankingCategory===nationalRankingState.category));document.querySelectorAll('[data-ranking-sex]').forEach(b=>b.classList.toggle('active',b.dataset.rankingSex===nationalRankingState.sex));document.querySelectorAll('[data-ranking-discipline]').forEach(b=>b.classList.toggle('active',b.dataset.rankingDiscipline===nationalRankingState.discipline));}
function setNationalRankingCategory(v){nationalRankingState.category=v==='general'?'general':'k2';syncNationalRankingButtons();loadNationalRanking();}
function setNationalRankingSex(v){nationalRankingState.sex=v==='女'?'女':'男';syncNationalRankingButtons();loadNationalRanking();}
function setNationalRankingDiscipline(v){nationalRankingState.discipline=['SL','GS','SG'].includes(v)?v:'SL';syncNationalRankingButtons();loadNationalRanking();}
function nationalRankingAthleteUrl(saj){const n=String(saj||'').replace(/\D/g,'');return /^\d{7,9}$/.test(n)?`https://sajdb.shikuminet.jp/alpine/biography/${encodeURIComponent(n)}`:'';}
function openNationalRankingAthlete(saj){const u=nationalRankingAthleteUrl(saj);if(u)window.open(u,'_blank','noopener,noreferrer');}
function renderNationalRanking(data){const body=document.getElementById('nationalRankingBody'),meta=document.getElementById('nationalRankingMeta'),status=document.getElementById('nationalRankingStatus');if(!body||!meta||!status)return;const rows=Array.isArray(data?.ranking)?data.ranking:[],cat=nationalRankingState.category==='k2'?'K2':'一般',sex=nationalRankingState.sex==='男'?'男子':'女子';meta.textContent=`${data?.seasonLabel||'シーズン—'} / ${data?.pointListNumber!=null?`SAJポイントリスト No.${data.pointListNumber}`:'ポイントリスト番号—'} / ${cat} / ${sex} / ${nationalRankingState.discipline}${data?.sourceMode==='CALENDAR_LIST_FALLBACK'?' / カレンダー同一No.補完':''}`;status.textContent=rows.length?`${rows.length}名を表示（ポイント未取得はランキング対象外）`:'該当するランキングデータがありません。';body.innerHTML=rows.map(r=>{const saj=String(r.saj||''),safeSaj=encodeURIComponent(saj),birth=r.birth?`Birth ${esc(r.birth)}`:'';return `<tr><td class="rank">${esc(String(r.rank??'—'))}</td><td><button class="national-ranking-athlete" type="button" onclick="openNationalRankingAthlete(decodeURIComponent('${safeSaj}'))">${esc(r.name||'氏名不明')}</button><div class="national-ranking-athlete-meta">SAJ ${esc(saj||'—')}${birth?` / ${birth}`:''}</div></td><td>${esc(r.organization||'—')}</td><td>${esc(r.team||'—')}</td><td class="point">${r.point!==null&&r.point!==undefined&&String(r.point).trim()!==''&&Number.isFinite(Number(r.point))?Number(r.point).toFixed(2):'—'}</td></tr>`}).join('');}
async function fetchNationalRankingCombo(category,sex,discipline){const q=new URLSearchParams({category,sex,discipline});const r=await fetch(`${apiBase()}/api/saj-ranking?${q}`,{cache:'no-store'});const data=await r.json().catch(()=>({}));if(!r.ok||!data?.ok)throw new Error(data?.detail?`${data.error||'取得失敗'}：${data.detail}`:(data?.error||`HTTP ${r.status}`));return data;}
async function refreshAllNationalRankings(manual=false){if(!navigator.onLine||!apiBase())return false;const status=document.getElementById('nationalRankingStatus');if(status)status.textContent=manual?'全国ランキングを一括更新しています…':'全国ランキングを週次更新しています…';const next=new Map(nationalRankingCache);let ok=true;for(const [category,sex,discipline] of NATIONAL_RANKING_COMBOS){try{next.set(nationalRankingKey({category,sex,discipline}),await fetchNationalRankingCombo(category,sex,discipline))}catch(e){ok=false;console.warn('全国TOP30一括取得失敗',category,sex,discipline,e);break}}if(ok){nationalRankingCache.clear();next.forEach((v,k)=>nationalRankingCache.set(k,v));persistNationalRankingCache()}else loadNationalRankingCacheFromStorage();const cur=nationalRankingCache.get(nationalRankingKey());if(cur)renderNationalRanking(cur);else if(status)status.textContent='ランキングを取得できませんでした。保存済みデータはありません。';return ok;}
async function loadNationalRanking(force=false){const body=document.getElementById('nationalRankingBody'),meta=document.getElementById('nationalRankingMeta'),status=document.getElementById('nationalRankingStatus');if(!body||!meta||!status)return;syncNationalRankingButtons();loadNationalRankingCacheFromStorage();if(force){await refreshAllNationalRankings(true);return}const data=nationalRankingCache.get(nationalRankingKey());if(data){renderNationalRanking(data);return}meta.textContent='全国ポイントランキング';status.textContent=nationalRankingAutoSeason()?'ランキングデータはまだ取得されていません。':'5月〜10月は自動更新しません。「更新」で一括取得できます。';body.innerHTML='<tr><td colspan="5" class="muted">保存済みランキングデータはありません。</td></tr>';}
async function autoRefreshNationalRankingsOnLaunch(){const stored=loadNationalRankingCacheFromStorage();if(nationalRankingNeedsWeeklyRefresh(stored))await refreshAllNationalRankings(false);}
...
function saveHomeScheduleView(view){
  homeScheduleViewMemory=view;
  try{localStorage.setItem(HOME_SCHEDULE_VIEW_KEY,view)}catch(e){}
}
...
function homeSupporterAnnualItems(){
  // v0.13.82:
  // HOMEカレンダーは buildSupporterAnnualCalendar() の結果だけを使用する。
  // v0.13.81では、この結果に db.schedules / db.events をもう一度足していたため、
  // 同じ予定が「Supporter年間カレンダー経由」と「Manager本体データ経由」の
  // 2経路から入り、1マス内で重複表示されるケースがあった。
  ...
  const add=x=>{
    ...
    // 表示上同じ予定は source に関係なく1件へ統合する。
    const key=[
      item.date,
      norm(item.title||item.kind),
      norm(item.place)
    ].join('|');
    if(seen.has(key))return;
    seen.add(key);
  ...
  try{
    ...
    console.error('Supporter年間カレンダーデータ生成エラー',e);
  }
...
const NATIONAL_RANK_CACHE_KEY='alpine_team_manager_national_rank_cache_v2';
...
function hasSupporterSyncSettings(){const s=getSupporterSyncSettings();return !!(s.teamId&&s.managerKey);}
...
async function publishSupporterData(){if(!hasSupporterSyncSettings()){const message='情報を更新するには、先に接続設定を保存してください。';renderSupporterSyncStatus(message,true);alert(message);return;}const button=document.getElementById('supporterPublishBtn');try{if(button)button.disabled=true;renderSupporterSyncStatus('Supporter向けの最新情報を更新しています…');const envelope=await buildSupporterUpdateEnvelope();const data=await supporterManagerRequest('/api/manager/publish',{method:'POST',body:JSON.stringify(envelope)});renderSupporterSyncStatus(`更新完了：バージョン ${data.version}（${new Date(data.publishedAt).toLocaleString('ja-JP')}）`);alert('Supporter向け情報を更新しました。登録済み端末は次回起動時に自動更新されます。');}catch(e){renderSupporterSyncStatus(e.message,true);alert('Supporter向け情報を更新できませんでした。\n'+e.message);}finally{if(button)button.disabled=false;}}
async function shareSupporterInviteLink(){if(!hasSupporterSyncSettings()){const message='招待URLを共有するには、先に接続設定を保存してください。';renderSupporterSyncStatus(message,true);alert(message);return;}try{renderSupporterSyncStatus('招待URLを発行しています…');const data=await supporterManagerRequest('/api/manager/invite',{method:'POST',body:JSON.stringify({expiresHours:168,maxUses:100})});const text=`Alpine Team Supporterの登録案内です。\n下の専用URLをコピーし、LINE内ではなくChromeやSafariなどのブラウザで開いてください。\nブラウザの「ホーム画面に追加」から、アプリとして追加してご利用ください。\n\n${data.inviteUrl}\n\n登録コード：${data.inviteCode}\n有効期限：${new Date(data.expiresAt).toLocaleString('ja-JP')}`;if(navigator.share){await navigator.share({title:'Alpine Team Supporter 登録案内',text});}else if(navigator.clipboard){await navigator.clipboard.writeText(text);alert('招待案内をコピーしました。LINEに貼り付けて送信してください。');}else{prompt('この招待案内をコピーして送信してください。',text);}renderSupporterSyncStatus('招待URLを発行しました。');}catch(e){if(e?.name==='AbortError')return;renderSupporterSyncStatus(e.message,true);alert('招待URLを発行できませんでした。\n'+e.message);}}
...
function backupFilename(){
  return `alpine-team-manager-backup-${new Date().toISOString().slice(0,10).replaceAll('-','')}.json`;
}
function makeBackupFile(){
  return new File([JSON.stringify(buildBackupEnvelope(),null,2)],backupFilename(),{type:'application/json;charset=utf-8'});
}
...
function markBackupCreated(){
  db.team=db.team||{};db.team.lastBackupAt=new Date().toISOString();persistDb({silent:true});renderBackupStatus();
}
function exportJson(){markBackupCreated();downloadBackupFile(makeBackupFile());}
async function shareBackupFile(){
  markBackupCreated();
  const file=makeBackupFile(),team=(db.team?.name||'Alpine Team Manager').trim();
  try{
    if(navigator.share && (!navigator.canShare || navigator.canShare({files:[file]}))){
      await navigator.share({title:'Alpine Team Manager バックアップ',text:`${team} のバックアップファイルです。`,files:[file]});
      return;
    }
    downloadBackupFile(file);
    alert('バックアップを保存しました。LINEの「＋」→「ファイル」からこのJSONファイルを送信してください。');
  }catch(e){
    if(e?.name==='AbortError')return;
    downloadBackupFile(file);
    alert('共有メニューを開けなかったため、バックアップを保存しました。LINEの「＋」→「ファイル」から送信してください。');
  }
}
...
function normalizeImportedDb(parsed){
  ...
  let source=parsed;
  if(parsed.schema===BACKUP_FILE_SCHEMA && parsed.data)source=parsed.data;
  else if(parsed.schema===BACKUP_SHARE_SCHEMA && parsed.data)source=parsed.data;
  else if(parsed.data && typeof parsed.data==='object' && (parsed.data.team||Array.isArray(parsed.data.athletes)||Array.isArray(parsed.data.events)))source=parsed.data;
  const clean=sanitizeImportedValue(source);
...
async function importJson(ev){
  ...
  try{
    if(f.size>10*1024*1024)throw new Error('ファイルが大きすぎます（上限10MB）');
    let raw=(await f.text()).replace(/^\uFEFF/,'').trim();
    if(!raw)throw new Error('ファイルが空です');
    let parsed=JSON.parse(raw);
    if(typeof parsed==='string' && parsed.trim().startsWith('{'))parsed=JSON.parse(parsed.trim());
    const clean=normalizeImportedDb(parsed);
    const ok=confirm(`バックアップを読み込みます。\n\nチーム：${clean.team?.name||'名称未設定'}\n選手：${clean.athletes.length}名\nコーチ：${clean.coaches.length}名\n大会：${clean.events.length}件\n年間予定：${clean.schedules.length}件\n\n現在の端末データを置き換えますか？`);
    if(!ok)return;
    db=clean;
    if(!persistDb())throw new Error('端末への保存に失敗しました');
    renderAll();alert('バックアップの読込が完了しました。');
  }catch(e){
    console.error('バックアップ読込失敗',e);
...
async function refreshAppCacheOnLaunch(){
  ...
  try{
    const reg=await navigator.serviceWorker.register('./sw.js?ver=01393',{updateViaCache:'none'});
    ...
    const warm=()=>reg.active?.postMessage({type:'WARM_APP_SHELL'});
    if(reg.active)warm();
    else navigator.serviceWorker.ready.then(ready=>ready.active?.postMessage({type:'WARM_APP_SHELL'})).catch(()=>{});
...
renderAll();
autoRefreshNationalRankingsOnLaunch().catch(()=>{});
...
async function applyMinorAppUpdate(latest){
  ...
  try{
    ...
    // Keep the newly built offline shell. The activated worker removes only
    // obsolete Manager caches after the new shell has been cached completely.
...
const MANAGER_ACCESS_API='https://alpine-supporter-api.take6583.workers.dev';
const MANAGER_ACCESS_DEVICE_KEY='alpine_team_manager_anonymous_device_v1';
const MANAGER_ACCESS_SESSION_KEY='alpine_team_manager_access_sent_v01393';
let managerAccessReporting=false;
function managerAnonymousDeviceId(){
  let id=localStorage.getItem(MANAGER_ACCESS_DEVICE_KEY)||'';
  if(!/^[a-zA-Z0-9._:-]{8,100}$/.test(id)){
    id=crypto.randomUUID?.()||`${Date.now()}-${Math.random().toString(36).slice(2)}`;
    localStorage.setItem(MANAGER_ACCESS_DEVICE_KEY,id);
  }
  return id;
}
function managerLaunchMode(){
  return (window.matchMedia?.('(display-mode: standalone)')?.matches||navigator.standalone===true)?'standalone':'browser';
}
async function reportAnonymousManagerAccess(){
  if(!navigator.onLine||managerAccessReporting||sessionStorage.getItem(MANAGER_ACCESS_SESSION_KEY))return;
  managerAccessReporting=true;
  try{
    const response=await fetch(`${MANAGER_ACCESS_API}/api/manager/access`,{
      method:'POST',
      headers:{'Content-Type':'application/json'},
      credentials:'omit',
      referrerPolicy:'no-referrer',
      body:JSON.stringify({deviceId:managerAnonymousDeviceId(),appVersion:APP_VERSION,mode:managerLaunchMode()})
    });
    if(response.ok)sessionStorage.setItem(MANAGER_ACCESS_SESSION_KEY,'1');
  }catch{}
  finally{managerAccessReporting=false;}
}
setTimeout(()=>checkLatestAppVersionOnLaunch(),1200);
setTimeout(()=>reportAnonymousManagerAccess(),1800);
addEventListener('online',()=>reportAnonymousManagerAccess());
